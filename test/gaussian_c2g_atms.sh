#! /usr/bin/env bash
#SBATCH --job-name=gaussian_c2g_atm
...
cd $DATA||exit 99
if [[ ! -d gaussian_atms$AFHR ]]; then
  rm -rf gaussian_atms$AFHR
fi
...
export OMP_NUM_THREADS=${OMP_NUM_THREADS_ATMS:-2}
...
APREFIX="gdas.t${hh}z."
ASUFFIX=".nc"
$NLN $COMOUT/${APREFIX}atmf$( printf "%03d" $RHR)${ASUFFIX} ./atmf${ASUFFIX}
$NLN $COMOUT/${APREFIX}atm${AFHR}${ASUFFIX} ./atm${AFHR}${ASUFFIX}
...
eval $GAUATMSEXE >> ${COMOUT}/log${AFHR}
