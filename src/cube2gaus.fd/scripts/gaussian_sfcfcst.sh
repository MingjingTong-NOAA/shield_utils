...
if [[ $RHR -eq 0 ]]; then 
   ...
   $NLN $DATA/RESTART/${rPDY}.${rcyc}${rmin}00.sfc_data.tile1.nc   ./fcst.tile1.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}${rmin}00.sfc_data.tile2.nc   ./fcst.tile2.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}${rmin}00.sfc_data.tile3.nc   ./fcst.tile3.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}${rmin}00.sfc_data.tile4.nc   ./fcst.tile4.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}${rmin}00.sfc_data.tile5.nc   ./fcst.tile5.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}${rmin}00.sfc_data.tile6.nc   ./fcst.tile6.nc
elif [[ $RHR -ne $REND ]] ; then
   $NLN $DATA/RESTART/${rPDY}.${rcyc}0000.sfc_data.tile1.nc   ./fcst.tile1.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}0000.sfc_data.tile2.nc   ./fcst.tile2.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}0000.sfc_data.tile3.nc   ./fcst.tile3.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}0000.sfc_data.tile4.nc   ./fcst.tile4.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}0000.sfc_data.tile5.nc   ./fcst.tile5.nc
   $NLN $DATA/RESTART/${rPDY}.${rcyc}0000.sfc_data.tile6.nc   ./fcst.tile6.nc
else
...
if [ ! -d $COMOUT/sfcfcst ]; then
  mkdir -p $COMOUT/sfcfcst
fi
$NLN $COMOUT/sfcfcst/${APREFIX}sfcf$( printf "%03d" $fhour)${ASUFFIX} ./sfc.gaussian.nc

eval $GAUSFCFCSTEXE >> $DATA/gaussian_c2g_sfcf$( printf "%03d" $RHR)_${exp}/logf$( printf "%03d" $fhour)
export ERR=$?
export err=$ERR
$ERRSCRIPT||exit 2
